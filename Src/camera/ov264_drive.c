...
static const uint8_t svga_regs[][2] = {
        ...
        { COM7,    COM7_RES_SVGA},//resolution selection SVGA   //by datasheet!!!
        { COM1,    0x0F }, /* UXGA=0x0F, SVGA=0x0A, CIF=0x06 */ // by datasheet!!!
        { REG32,   0x09 }, /* UXGA=0x36, SVGA/CIF=0x09 */       // by datasheet!!!

        { HSTART,  0x11 }, /* UXGA=0x11, SVGA/CIF=0x11 */       // by datasheet!!!
        { HSTOP,   0x43 }, /* UXGA=0x75, SVGA/CIF=0x43 */       // by datasheet!!!

        { VSTART,  0x00 }, /* UXGA=0x01, SVGA/CIF=0x00 */       // by datasheet!!!
        ...
        { 0x22,    0x1a },//RESERVED?
        { 0x37,    0xc3 },//RESERVED?
        { 0x34,    0xc0 },//WRITE IN RESERVED?
        { 0x06,    0x88 },//RESERVED?
        { 0x0d,    0x87 },//WRITE IN RESERVED?
        { 0x0e,    0x41 },//RESERVED?
        { 0x42,    0x03 },//RESERVED?
        ...
        { RESET,   RESET_DVP },
        { HSIZE8,  (SVGA_HSIZE>>3)}, /* Image Horizontal Size HSIZE[10:3] */ //800>>3=100
        { VSIZE8,  (SVGA_VSIZE>>3)}, /* Image Vertiacl Size VSIZE[10:3] */   //600>>3=75
        ...
        { HSIZE,   ((SVGA_HSIZE>>2)&0xFF) }, /* H_SIZE[7:0]= HSIZE/4 *///800>>2=200
        { VSIZE,   ((SVGA_VSIZE>>2)&0xFF) }, /* V_SIZE[7:0]= VSIZE/4 *///600>>2=150
        ...
        { CTRL2,   CTRL2_DCW_EN | CTRL2_SDE_EN |
          CTRL2_UV_AVG_EN | CTRL2_CMX_EN | CTRL2_UV_ADJ_EN },//+DCW ,SDE?

        /* H_DIVIDER/V_DIVIDER */
        { CTRLI,   CTRLI_LP_DP | 0x00},//LP_DP?
        /* DVP prescalar */
        { R_DVP_SP, R_DVP_SP_AUTO_MODE},//AUTO_MODE?
...
static const uint8_t jpeg_regs[][2] = {
        { BANK_SEL, BANK_SEL_DSP },//select table
        { RESET,   RESET_DVP},     //reset digital video port
        { IMAGE_MODE, IMAGE_MODE_JPEG_EN|IMAGE_MODE_RGB565 },//output format jpeg
        { 0xD7,     0x03 },
        { 0xE1,     0x77 },
        { QS,       0x0C },
        { RESET,    0x00 },
...
static int reset(sensor_t *sensor)
{
    ...
    //SCCB_Write(sensor->slv_addr, COM7, COM7_SRST);
	   DCMI_SingleRandomWrite(BANK_SEL, BANK_SEL_SENSOR);
	   DCMI_SingleRandomWrite(COM7, COM7_SRST);
    /* delay n ms */
    HAL_Delay(10);
    ...
    while (regs[i][0]) {
        //SCCB_Write(sensor->slv_addr, regs[i][0], regs[i][1]);
			  DCMI_SingleRandomWrite(regs[i][0], regs[i][1]);
        i++;
    }
...
static int set_pixformat(sensor_t *sensor, pixformat_t pixformat)
{
    ...
    switch (pixformat) {
        ...
        case PIXFORMAT_GRAYSCALE:
            regs = yuv422_regs;
            break;
        case PIXFORMAT_JPEG://PIXFORMAT_JPEG
            regs = jpeg_regs;
            break;
        default:
            return -1;
...
static int set_framesize(sensor_t *sensor, framesize_t framesize)
{
    ...
    if (framesize <= FRAMESIZE_SVGA) {
        clkrc =0x83;//10000011  16/4=8mhz?
        regs = svga_regs;
    } else {
        clkrc =0x87;
        regs = uxga_regs;//16/8=2mhz?
    }
...
//    ret |= SCCB_Write(sensor->slv_addr, R_BYPASS, R_BYPASS_DSP_BYPAS);
		  DCMI_SingleRandomWrite(BANK_SEL, BANK_SEL_DSP);//table 0
		  DCMI_SingleRandomWrite(R_BYPASS, R_BYPASS_DSP_BYPAS);//bypas DSP sensor out directly
...
//    ret |= SCCB_Write(sensor->slv_addr, ZMHH, ((h>>8)&0x04)|((w>>10)&0x03)); // OUTH[8]/OUTW[9:8]
      DCMI_SingleRandomWrite(ZMOW, (w>>2)&0xFF);//800>>2=200
		  DCMI_SingleRandomWrite(ZMOH, (h>>2)&0xFF);//600>>2=150
		  DCMI_SingleRandomWrite(ZMHH,((h>>8)&0x04)|((w>>10)&0x03));//0
    /* Set CLKRC */
//    ret |= SCCB_Write(sensor->slv_addr, BANK_SEL, BANK_SEL_SENSOR);
//    ret |= SCCB_Write(sensor->slv_addr, CLKRC, clkrc);
		  DCMI_SingleRandomWrite(BANK_SEL, BANK_SEL_SENSOR);
      DCMI_SingleRandomWrite(CLKRC, clkrc);
    /* Write DSP input regsiters */
    while (regs[i][0]) {
        //SCCB_Write(sensor->slv_addr, regs[i][0], regs[i][1]);
			  DCMI_SingleRandomWrite(regs[i][0], regs[i][1]);
...
//    ret |= SCCB_Write(sensor->slv_addr, R_BYPASS, R_BYPASS_DSP_EN);
		  DCMI_SingleRandomWrite(BANK_SEL, BANK_SEL_DSP);
		  DCMI_SingleRandomWrite(R_BYPASS, R_BYPASS_DSP_EN);
    /* delay n ms */
    HAL_Delay(30);
...
static int set_contrast(sensor_t *sensor, int level)
{
    ...
    for (i=0; i<sizeof(contrast_regs[0])/sizeof(contrast_regs[0][0]); i++) {
        //ret |= SCCB_Write(sensor->slv_addr, contrast_regs[0][i], contrast_regs[level][i]);
			  DCMI_SingleRandomWrite(contrast_regs[0][i], contrast_regs[level][i]);
    }
...
static int set_brightness(sensor_t *sensor, int level)
{
    ...
    for (i=0; i<sizeof(brightness_regs[0])/sizeof(brightness_regs[0][0]); i++) {
        //ret |= SCCB_Write(sensor->slv_addr, brightness_regs[0][i], brightness_regs[level][i]);
			  DCMI_SingleRandomWrite(brightness_regs[0][i], brightness_regs[level][i]);
    }
...
static int set_saturation(sensor_t *sensor, int level)
{
    ...
    for (i=0; i<sizeof(saturation_regs[0])/sizeof(saturation_regs[0][0]); i++) {
        //ret |= SCCB_Write(sensor->slv_addr, saturation_regs[0][i], saturation_regs[level][i]);
			  DCMI_SingleRandomWrite(saturation_regs[0][i], saturation_regs[level][i]);
    }
...
static int set_gainceiling(sensor_t *sensor, gainceiling_t gainceiling)
{
    ...
    DCMI_SingleRandomWrite( COM9, COM9_AGC_SET(gainceiling));
    return ret;
}
...
static int set_colorbar(sensor_t *sensor, int enable)
{
    ...
    DCMI_SingleRandomWrite(BANK_SEL, BANK_SEL_SENSOR);
    /* Update COM7 */
    DCMI_SingleRandomRead(COM7,&reg);//reg = SCCB_Read(sensor->slv_addr, COM7);
    ...
    //ret |= SCCB_Write(sensor->slv_addr, COM7, reg);
		DCMI_SingleRandomWrite(COM7, reg);
    return ret;
}
...
static int set_whitebal(sensor_t *sensor, int enable)
{
    ...
    DCMI_SingleRandomWrite(BANK_SEL, BANK_SEL_DSP);
    /* Update CTRL1 */
    DCMI_SingleRandomRead(CTRL1,&reg);//reg = SCCB_Read(sensor->slv_addr, CTRL1);
    ...
    //ret |= SCCB_Write(sensor->slv_addr, CTRL1, reg);
		DCMI_SingleRandomWrite(CTRL1, reg);
    return ret;
}
...
static int set_gain_ctrl(sensor_t *sensor, int enable)
{
    ...
    DCMI_SingleRandomWrite(BANK_SEL, BANK_SEL_SENSOR);
    /* Update COM8 */
    DCMI_SingleRandomRead(COM8,&reg);//reg = SCCB_Read(sensor->slv_addr, COM8);
    ...
    //ret |= SCCB_Write(sensor->slv_addr, COM8, reg);
		DCMI_SingleRandomWrite(COM8, reg);
    return ret;
}
...
static int set_hmirror(sensor_t *sensor, int enable)
{
    ...
    /* Update REG04 */
    DCMI_SingleRandomRead(REG04,&reg);//reg = SCCB_Read(sensor->slv_addr, REG04);
    ...
    //ret |= SCCB_Write(sensor->slv_addr, REG04, reg);
		DCMI_SingleRandomWrite(REG04, reg);
    return ret;
}
...
void esp32cam_ini(void){
	sensor_t sensor;
	//sensor.id.PID=0x26;
	//sensor.id.VER=0x42;
	sensor.slv_addr=0x30;
	//sensor.id.MIDL=0x72; //MIDL=0x72;
	//sensor.id.MIDH=0xA2; //MIDH=0xA2;
  //sensor.pixformat=PIXFORMAT_JPEG;
  //sensor. framesize=FRAMESIZE_SVGA;
	DCMI_SingleRandomRead(sensor.slv_addr,&sensor.id.PID);
	DCMI_SingleRandomRead(sensor.slv_addr,&sensor.id.VER);
	reset(&sensor);
        set_pixformat(&sensor, PIXFORMAT_RGB565);//sensor
        set_framesize(&sensor,FRAMESIZE_SVGA);//sensor 160x120
	set_quality(&sensor,10);//set_quality register QS 0x44//10
