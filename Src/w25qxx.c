...
void SPI_FLASH_BufferWrite(uint8_t* pBuffer, uint32_t WriteAddr, uint32_t NumByteToWrite)
{
  ...
  Addr = WriteAddr %  w25qxx.PageSize; // SPI_FLASH_PageSize;
  count = w25qxx.PageSize - Addr;
  NumOfPage =  NumByteToWrite / w25qxx.PageSize;
  NumOfSingle = NumByteToWrite % w25qxx.PageSize;

  if (Addr == 0) /* WriteAddr is SPI_FLASH_PageSize aligned  */
  {
    if (NumOfPage == 0) /* NumByteToWrite < SPI_FLASH_PageSize */
    {
      W25qxx_WritePage(pBuffer, WriteAddr,0, NumByteToWrite);
    }
    else /* NumByteToWrite > SPI_FLASH_PageSize */
    {
      while (NumOfPage--)
      {
        W25qxx_WritePage(pBuffer, WriteAddr,0, w25qxx.PageSize);
        WriteAddr +=  w25qxx.PageSize;
        pBuffer += w25qxx.PageSize;
      }

      W25qxx_WritePage(pBuffer, WriteAddr,0, NumOfSingle);
    }
  }
  else /* WriteAddr is not SPI_FLASH_PageSize aligned  */
  {
    if (NumOfPage == 0) /* NumByteToWrite < SPI_FLASH_PageSize */
    {
      if (NumOfSingle > count) /* (NumByteToWrite + WriteAddr) > SPI_FLASH_PageSize */
      {
        ...
        W25qxx_WritePage(pBuffer, WriteAddr,0, count);
        WriteAddr +=  count;
        pBuffer += count;

        W25qxx_WritePage(pBuffer, WriteAddr,0, temp);
      }
      else
      {
        W25qxx_WritePage(pBuffer, WriteAddr,0, NumByteToWrite);
