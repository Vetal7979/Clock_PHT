...
{
  /* Удалим предыдущий файл с диска */
 // f_unlink (REC_WAVE_NAME);
  ...
  /* Записываем хедер в файл */
  f_write(&WavFile, pHeaderBuff, 44, &byteswritten);

  /* Увеличим счетчик файла */
...
{
    // конец записи
  /* Обновим данные в хедере файла */
  f_lseek(&WavFile, 0);
  /* пропишем в хедер необходимую информацию */
...
{
  st_t = 30000;
   rec_file_make(); // создали заголовок


   while(st_t)
  {
     // 0 - нужно писать начало буфера 1 - середину
     if(full == 1)  // заполнен весь буфер?
      {
       full=0;
       f_write(&WavFile, &RecBuff[2304/2],2304, &byteswritten); // записываем из серединки
       BufferCtl.fptr += byteswritten;
      }
     if (half == 1)
      {
       half = 0;
       f_write(&WavFile, &RecBuff[0],2304, &byteswritten); // записываем сначала
       BufferCtl.fptr += byteswritten;
      }
   }

     final_rec_file(); // финализируем файл
